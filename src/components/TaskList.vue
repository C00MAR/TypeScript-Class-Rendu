  ...
  <div class="task-manager p-4">
    <div class="mb-4">
      ...
      <form @submit.prevent="addTask" class="mb-4">
        <input v-model="newTask.title" class="border p-2 mr-2 rounded" placeholder="Titre" required />
        <input v-model="newTask.description" class="border p-2 mr-2 rounded" placeholder="Description" required />
        <button type="submit" class="bg-blue-500 text-white px-4 py-2 rounded">Ajouter</button>
      </form>

      <div class="mb-4">
        <button @click="setSortStrategy(new AlphabeticalSort())" class="bg-gray-200 px-3 py-1 rounded mr-2">
          Tri Alphabétique
        </button>
        <button @click="setSortStrategy(new CompletionSort())" class="bg-gray-200 px-3 py-1 rounded">
          Tri par État
        </button>
      </div>

      <div class="space-y-2">
        <div v-for="task in store.sortedTasks" :key="task.id"
          class="border p-3 rounded flex items-center justify-between" :class="{ 'bg-gray-100': task.completed }">
          <div>
